...
A sample render script that configures Falcor to use this render pass is in:
   `Source\SDURender\Data\RTXDI.py`
...
2) Run SDURender.exe, which allows you to load Falcor scenes and render scripts.

3) Load the RTXDI.py render script from one of the locations listed above (you can do this
via the SDURender command line, from the menu after running SDURender, or dragging & dropping the
.py file into a running SDURender window).
...
You can (also) load this scene via the SDURender command line, from the menu after running SDURender,
or by dragging and dropping the `.pyscene` into a running SDURender window.
